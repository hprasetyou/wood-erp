...
abstract class Currency implements ActiveRecordInterface
{
    ...
    public function setActive($v)
    {
        ...
        if ($this->active !== $v) {
            $this->active = $v;
            $this->modifiedColumns[CurrencyTableMap::COL_ACTIVE] = true;
        }
    ...
    public function setRounding($v)
    {
        ...
        if ($this->rounding !== $v) {
            $this->rounding = $v;
            $this->modifiedColumns[CurrencyTableMap::COL_ROUNDING] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : CurrencyTableMap::translateFieldName('Active', TableMap::TYPE_PHPNAME, $indexType)];
            $this->active = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : CurrencyTableMap::translateFieldName('Rounding', TableMap::TYPE_PHPNAME, $indexType)];
            $this->rounding = (null !== $col) ? (double) $col : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CurrencyTableMap::COL_ACTIVE)) {
            $modifiedColumns[':p' . $index++]  = 'active';
        }
        if ($this->isColumnModified(CurrencyTableMap::COL_ROUNDING)) {
            $modifiedColumns[':p' . $index++]  = 'rounding';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'active':
                        ...
                        break;
                    case 'rounding':
                        $stmt->bindValue($identifier, $this->rounding, PDO::PARAM_STR);
                        break;
                    case 'created_at':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CurrencyTableMap::COL_ACTIVE)) {
            $criteria->add(CurrencyTableMap::COL_ACTIVE, $this->active);
        }
        if ($this->isColumnModified(CurrencyTableMap::COL_ROUNDING)) {
            $criteria->add(CurrencyTableMap::COL_ROUNDING, $this->rounding);
        }
        if ($this->isColumnModified(CurrencyTableMap::COL_CREATED_AT)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setRounding($this->getRounding());
     ...
     * @return ObjectCollection|ChildProformaInvoice[] List of ChildProformaInvoice objects
     */
    public function getProformaInvoicesJoinDownPayment(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildProformaInvoiceQuery::create(null, $criteria);
